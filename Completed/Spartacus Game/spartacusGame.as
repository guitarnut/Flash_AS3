package {
	...
	public class spartacusGame extends MovieClip {
		...
		private function startPreloader():void {
			this.loaderInfo.addEventListener(ProgressEvent.PROGRESS, updatePreloader);
		}
		private function updatePreloader(event:ProgressEvent):void {
			preloader.loadText.text = String(Math.round((event.bytesLoaded/event.bytesTotal)*100))+"%";
			if (event.bytesLoaded==event.bytesTotal) {
				this.loaderInfo.removeEventListener(ProgressEvent.PROGRESS, updatePreloader);
				buildGameScreens();
				setupAudio();
			}
		}
